...
def build_ui():
    ...
    with gr.Blocks(title="Kamal Gemini Web Crawler & Q&A", css=PRO_CSS) as demo:
        ...
        with gr.Tab("🌐 Scrape & Index"):
            ...
            # Multiselect dropdown: selected links appear as blue chips
            ...
            # Live scraping log — Google blue text on dark background
        ...
        with gr.Tab("💬 Ask Questions"):
            ...
            question_input = gr.Textbox(
                label       = "Your Question",
                placeholder = "e.g. What are the capabilities of Gemini 2.0 Flash?",
                lines       = 3,
            ...
            with gr.Row():
                ...
                model_dropdown = gr.Dropdown(
                    ...
                    choices = [
                        "gemini-2.0-flash",      # Latest Gemini 2.0 — fastest, best for most tasks
                        "gemini-1.5-pro",         # Most capable Gemini 1.5, 1M token context
                        "gemini-1.5-flash",       # Balanced speed & quality (Gemini 1.5)
                        "gemini-1.5-flash-8b",    # Smallest Gemini — high-volume simple queries
                    ],
            ...
            with gr.Accordion("📚 Retrieved Source Documents", open=False):
                gr.Markdown(
                    ...
                    "and passed to Google Gemini as context._"
                )
        ...
        # Ask button → run Gemini RAG chain
        ask_btn.click(
            fn      = on_ask_question,
...
if __name__ == "__main__":
    """
    Launch the Google Gemini-powered Gradio web application.

    Access the UI at: http://localhost:7864
    Port 7864 is used so this can run alongside:
      ...
      - webpage_query_openai_ui.py   → port 7863  (OpenAI)
    """
    app = build_ui()
    app.launch(
        server_name = "0.0.0.0",     # Listen on all network interfaces
        server_port = 7864,           # Unique port for the Gemini UI
