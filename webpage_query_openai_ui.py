# =====================================================================
# FILE: webpage_query_openai_ui.py
# =====================================================================
# Kamal AI Web Crawler & Question Answering — OpenAI Edition
# =====================================================================
#
# PURPOSE:
#   Same workflow as webpage_query_groq_ui.py, but uses OpenAI's GPT
#   models as the LLM instead of Groq. Uses ChatOpenAI from
#   langchain_openai, which reads OPENAI_API_KEY from the .env file.
#
# WORKFLOW:
...
#   - LLM:          OpenAI API (gpt-4o, gpt-4o-mini, gpt-4-turbo, gpt-3.5-turbo)
...
#   - Uses langchain_openai.ChatOpenAI (official OpenAI endpoint)
#   - Models available:
#       gpt-4o        → Latest flagship, best reasoning + multimodal (recommended)
...
#
# USAGE:
#   python webpage_query_openai_ui.py
#   Then open http://localhost:7863 in your browser
#   (Port 7863 avoids conflict with Groq:7860, DeepSeek:7861, Claude:7862)
# =====================================================================
...
# No base_url override needed — this connects to the official OpenAI API.
from langchain_openai import ChatOpenAI

# Load OPENAI_API_KEY and other secrets from the .env file
...
def discover_links(base_url: str) -> tuple[list[str], str]:
    ...
    try:
        ...
        base_domain = tldextract.extract(base_url).domain  # e.g. "openai" from "platform.openai.com"
...
def scrape_urls_streaming(urls: list[str]) -> Generator:
    ...
    # Save to a separate file — does not overwrite Groq/DeepSeek/Claude versions
    with open("scraped_data_openai.txt", "w", encoding="utf-8") as f:
        f.write(cleaned_text)
    log += "💾 Saved to scraped_data_openai.txt\n"
...
# OpenAI GPT models handle all four chain types with the same templates.
...
# =====================================================================
# SECTION 4: QA CHAIN BUILDER  (LCEL — OpenAI edition)
# =====================================================================
# Same LCEL logic as webpage_query_groq_ui.py.
# The ONLY difference: LLM is ChatOpenAI pointed at the official
# OpenAI API instead of ChatGroq.
...
def _run_stuff_chain(retriever, llm, question: str) -> tuple[str, list]:
    ...
    context block and sent to the OpenAI model in a single prompt.
...
def _run_map_reduce_chain(retriever, llm, question: str) -> tuple[str, list]:
    """
    MAP-REDUCE strategy — each chunk is answered by the OpenAI model
    independently (map), then all partial answers are combined into
    one final answer (reduce).

    Best for: many pages / large volumes of retrieved text.
    """
    docs = retriever.invoke(question)

    # MAP: model answers each chunk individually
...
def _run_map_rerank_chain(retriever, llm, question: str) -> tuple[str, list]:
    """
    MAP-RERANK strategy — the OpenAI model scores each chunk
    independently (0–10). The answer from the highest-scoring
    chunk is returned as the final result.
...
def run_qa(vectorstore, model_name: str, chain_type: str, question: str) -> tuple[str, list]:
    """
    Dispatcher: initialises the OpenAI LLM, selects the correct LCEL
    chain strategy, and returns the answer with its source documents.

    ── OpenAI LLM setup ─────────────────────────────────────────────
    ChatOpenAI reads OPENAI_API_KEY from the environment automatically.
    No base_url override is needed — this connects to api.openai.com.

    Available models:
      gpt-4o        Latest flagship — best reasoning, vision support
      gpt-4o-mini   Fast & affordable — great balance for most tasks
...
def on_ask_question(question: str, model_name: str, chain_type: str, vectorstore):
    ...
    Runs the user's question through the OpenAI-powered RAG chain and
...
def build_ui():
    ...
    Identical layout to webpage_query_groq_ui.py with these differences:
      - Header colour: emerald green accent to distinguish from
          Groq (navy), DeepSeek (teal), and Claude (purple) UIs
      - Badge mentions OpenAI / GPT
      - Model dropdown lists OpenAI GPT model IDs
      - Port 7863
    ...
    # ── Professional CSS — White / Black / Emerald Green (OpenAI edition) ──
    PRO_CSS = """
    ...
    /* ── Hero header — OpenAI green ── */
    .app-header {
        background: linear-gradient(135deg, #064e3b 0%, #065f46 50%, #10b981 100%);
        border-radius: 10px;
        padding: 26px 32px;
        margin-bottom: 4px;
        box-shadow: 0 2px 12px rgba(6,95,70,0.20);
    }
    .app-header h1 {
    ...
    /* Selected link chips — emerald green */
    ...
    button.primary {
        ...
        box-shadow: 0 1px 6px rgba(5,150,105,0.30) !important;
    ...
    /* ── Live log box (dark terminal, green text) ── */
    .log-box textarea {
    ...
    with gr.Blocks(title="Kamal OpenAI Web Crawler & Q&A", css=PRO_CSS) as demo:
        ...
        <div class="app-header">
            ...
            <p>Scrape any website &rarr; Index its content &rarr; Ask questions using OpenAI GPT</p>
            <div class="badge-row">
        ...
        with gr.Tab("🌐 Scrape & Index"):
            ...
            with gr.Row():
                url_input = gr.Textbox(
                    label       = "Website URL",
                    placeholder = "https://platform.openai.com/docs/overview",
                    scale       = 4,
            ...
            # Multiselect dropdown: selected links appear as green chips
            ...
            # Live scraping log — green text on dark background
        ...
        with gr.Tab("💬 Ask Questions"):
            ...
            question_input = gr.Textbox(
                label       = "Your Question",
                placeholder = "e.g. What are the key features of GPT-4o?",
                lines       = 3,
            ...
            with gr.Row():
                ...
                model_dropdown = gr.Dropdown(
                    label   = "OpenAI Model",
                    choices = [
                        "gpt-4o",           # Latest flagship — best reasoning & multimodal
                        "gpt-4o-mini",      # Fast & affordable — great for most tasks
                        "gpt-4-turbo",      # Previous generation flagship, 128k context
                        "gpt-3.5-turbo",    # Fastest & cheapest — simple queries
                    ],
                    value = "gpt-4o-mini",
                    info  = (
                        "gpt-4o = most capable | "
            ...
            with gr.Accordion("📚 Retrieved Source Documents", open=False):
                gr.Markdown(
                    ...
                    "and passed to OpenAI GPT as context._"
                )
        ...
        # Ask button → run OpenAI RAG chain
        ask_btn.click(
            fn      = on_ask_question,
...
if __name__ == "__main__":
    """
    Launch the OpenAI-powered Gradio web application.

    Access the UI at: http://localhost:7863
    Port 7863 is used so this can run alongside:
      - webpage_query_groq_ui.py     → port 7860  (Groq)
      - webpage_query_deepseek_ui.py → port 7861  (DeepSeek)
      - webpage_query_claude_ui.py   → port 7862  (Claude)
    """
    app = build_ui()
    app.launch(
        server_name = "0.0.0.0",     # Listen on all network interfaces
        server_port = 7863,           # Unique port for the OpenAI UI
