# =====================================================================
# FILE: webpage_query_claude_ui.py
# =====================================================================
# Kamal AI Web Crawler & Question Answering — Claude (Anthropic) Edition
# =====================================================================
#
# PURPOSE:
#   Same workflow as webpage_query_ui.py, but uses Anthropic's Claude
#   as the LLM instead of Groq. Uses ChatAnthropic from langchain_anthropic,
#   which reads ANTHROPIC_API_KEY from the .env file.
#
# WORKFLOW:
...
#   - LLM:          Anthropic Claude API (claude-opus-4-6, claude-sonnet-4-6, claude-haiku-4-5)
...
#   - Models (latest as of 2025):
#       claude-opus-4-6    → Most capable, best for complex reasoning
#       claude-sonnet-4-6  → Balanced speed and intelligence (recommended)
#       claude-haiku-4-5   → Fastest and most compact
#
# USAGE:
#   python webpage_query_claude_ui.py
#   Then open http://localhost:7862 in your browser
#   (Port 7862 avoids conflict with Groq UI on 7860 and DeepSeek UI on 7861)
# =====================================================================
...
# ChatAnthropic automatically reads ANTHROPIC_API_KEY from the environment.
from langchain_anthropic import ChatAnthropic

# Load ANTHROPIC_API_KEY and other secrets from the .env file
...
def discover_links(base_url: str) -> tuple[list[str], str]:
    ...
    try:
        ...
        base_domain = tldextract.extract(base_url).domain  # e.g. "anthropic" from "docs.anthropic.com"
...
def scrape_urls_streaming(urls: list[str]) -> Generator:
    ...
    # Save to a separate file so it doesn't overwrite the Groq/DeepSeek versions
    with open("scraped_data_claude.txt", "w", encoding="utf-8") as f:
        f.write(cleaned_text)
    log += "💾 Saved to scraped_data_claude.txt\n"
...
# Claude handles all four chain types with the same template structure.
...
# =====================================================================
# SECTION 4: QA CHAIN BUILDER  (LCEL — Claude edition)
# =====================================================================
# Same LCEL logic as webpage_query_ui.py.
# The ONLY difference: LLM is ChatAnthropic instead of ChatGroq.
...
def _run_stuff_chain(retriever, llm, question: str) -> tuple[str, list]:
    ...
    context block and sent to Claude in a single prompt.
...
def _run_map_reduce_chain(retriever, llm, question: str) -> tuple[str, list]:
    """
    MAP-REDUCE strategy — each chunk is answered by Claude independently
    (map), then all partial answers are combined into a final answer (reduce).

    Best for: many pages / large volumes of retrieved text.
    """
    docs = retriever.invoke(question)

    # MAP: Claude answers each chunk individually
...
def _run_refine_chain(retriever, llm, question: str) -> tuple[str, list]:
    ...
    Best for: thorough, high-quality answers — Claude excels at this
    because it can carry context across iterations effectively.
    """
    docs = retriever.invoke(question)
    if not docs:
...
def _run_map_rerank_chain(retriever, llm, question: str) -> tuple[str, list]:
    """
    MAP-RERANK strategy — Claude scores each chunk independently (0–10).
...
def run_qa(vectorstore, model_name: str, chain_type: str, question: str) -> tuple[str, list]:
    """
    Dispatcher: initialises the Claude LLM, selects the correct LCEL
    ...
    Available models (latest as of 2025):
      ...
      claude-haiku-4-5   Fastest & most compact — good for simple queries
    ─────────────────────────────────────────────────────────────────
    ...
    Args:
        ...
        model_name  (str):   Claude model ID.
    ...
    """
    # ── Initialise Claude via langchain_anthropic ─────────────────────
...
def on_ask_question(question: str, model_name: str, chain_type: str, vectorstore):
    ...
    Runs the user's question through the Claude-powered RAG chain and
...
def build_ui():
    ...
    Identical layout to webpage_query_ui.py with these visual differences:
      - Header colour: rich purple accent to distinguish from Groq (navy)
        and DeepSeek (teal) UIs
      - Badge mentions Claude / Anthropic
      - Model dropdown lists Claude model IDs

    Returns:
        gr.Blocks: The fully configured Gradio application.
    """

    # ── Professional CSS — White / Black / Purple (Claude edition) ────
    PRO_CSS = """
    ...
    /* ── Hero header — purple accent for Claude ── */
    .app-header {
        background: linear-gradient(135deg, #4c1d95 0%, #6d28d9 60%, #7c3aed 100%);
        border-radius: 10px;
        padding: 26px 32px;
        margin-bottom: 4px;
        box-shadow: 0 2px 12px rgba(109,40,217,0.20);
    }
    .app-header h1 {
    ...
    button.primary {
        ...
        box-shadow: 0 1px 6px rgba(124,58,237,0.3) !important;
    ...
    button.secondary:hover {
        ...
        box-shadow: 0 1px 6px rgba(124,58,237,0.15) !important;
    }

    /* ── Live log box (dark terminal, purple tint) ── */
    .log-box textarea {
    ...
    with gr.Blocks(title="Kamal Claude Web Crawler & Q&A", css=PRO_CSS) as demo:
        ...
        <div class="app-header">
            ...
            <p>Scrape any website &rarr; Index its content &rarr; Ask questions using Claude AI</p>
            <div class="badge-row">
        ...
        with gr.Tab("🌐 Scrape & Index"):
            ...
            # Multiselect dropdown: selected links appear as purple chips
            ...
            # Live log — purple text on dark background
        ...
        with gr.Tab("💬 Ask Questions"):
            ...
            question_input = gr.Textbox(
                label       = "Your Question",
                placeholder = "e.g. What are Claude's key capabilities?",
                lines       = 3,
            ...
            with gr.Row():
                ...
                # claude-opus-4-6    → Most capable, best for complex reasoning
                # claude-sonnet-4-6  → Balanced speed & intelligence (recommended)
                # claude-haiku-4-5   → Fastest, lightest — good for simple queries
                model_dropdown = gr.Dropdown(
                    label   = "Claude Model",
                    choices = [
                        "claude-opus-4-6",    # Most capable — complex reasoning & analysis
                        "claude-sonnet-4-6",  # Balanced — speed + intelligence (recommended)
                        "claude-haiku-4-5",   # Fastest — lightweight, simple queries
                    ],
                ...
                chain_type_dropdown = gr.Dropdown(
                    ...
                    choices = [
                        ...
                        "refine",       # Answer improved across chunks (Claude excels here)
            ...
            with gr.Accordion("📚 Retrieved Source Documents", open=False):
                gr.Markdown(
                    ...
                    "and passed to Claude as context._"
                )
        ...
        # Keep chunks number display in sync with state
        ...
        # Ask button → run Claude RAG chain
        ask_btn.click(
            fn      = on_ask_question,
...
if __name__ == "__main__":
    """
    Launch the Claude-powered Gradio web application.

    Access the UI at: http://localhost:7862
    Port 7862 is used so this can run alongside:
      - webpage_query_ui.py          → port 7860  (Groq)
      - webpage_query_deepseek_ui.py → port 7861  (DeepSeek)
    """
    app = build_ui()
    app.launch(
        server_name = "0.0.0.0",     # Listen on all network interfaces
        server_port = 7862,           # Unique port for the Claude UI
